...
var TOOL = {
	...
	getindex:function(dom,type,offset){
        ...
        switch(type){
        	case 'new':
        		post_url = "http://app..com/api/app_art.php?act=index&t=new";
        		break;
        	case 'down':
        		post_url = "http://app..com/api/app_art.php?act=index&t=down";
        		break;
        	case 'zan':
        		post_url = "http://app..com/api/app_art.php?act=index&t=zan";
        		break;
        }
        if(post_url)
	        $.ajax({
	                ...
	                success: function(data) {
	                	if(data.status==1){
		                	...
		                	for(i in data){
		                		...
		                		url = data[i]['jumpUrl'];
		                		name = data[i]['zhName']?data[i]['zhName']:data[i]['usName'];
  	...
  	getGame:function(e) {
  		...
  		,post_url = "http://app..com/api/app_art.php?act=category"
                ...
                success: function(data) {
                	if(data && data.status==1){
	                	...
	                	for(i in data){
	                		...
	                		url = data[i]['jumpUrl'];
		                	name = data[i]['zhName']?data[i]['zhName']:data[i]['usName'];
	                		str += '<li><a href="'+url+'" title="'+name+'"><span><img src="'+data[i]['downIcon']+'"></span><b>'+name+'</b><p>'+data[i]['types']+'</p></a></li>';
	                	}
  	...
  	getSearch:function(e) {
  		...
  		,post_url = "http://app..com/api/app_art.php?act=search"
  		,offset = typeof(arguments[0])=='number'?arguments[0]:0
  		,str = ''
  		,i
  		,_offset = 0
                ...
                success: function(data) {
                	if(data && data.status==1){
	                	...
	                	for(i in data){
		                	...
		                	name = data[i]['zhName']?data[i]['zhName']:data[i]['usName'];
		                	if(i<5)
		                		hotStr += '<li><a href="'+url+'" title="'+name+'"><span><img src="'+data[i]['downIcon']+'"><strong></strong></span><b>'+name+'</b><p>'+data[i]['types']+'</p></a></li>';
		                	else
	                			str += '<li><a href="'+url+'" title="'+name+'"><span><img src="'+data[i]['downIcon']+'"></span><b>'+name+'</b><p>'+data[i]['types']+'</p></a></li>';
	                	}
  	...
  	getBibei:function(e) {
  		...
  		,post_url = "http://app..com/api/app_art.php?act=position"
  		,offset = typeof(arguments[0])=='number'?arguments[0]:0
  		,str = ''
  		,hotStr = ''
  		,i
