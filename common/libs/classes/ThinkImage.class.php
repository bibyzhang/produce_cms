...
class ThinkImage{
     ...
     * @return Object             当前图片处理库对象
     */
    public function save($savepath = './', $imgname, $type = null, $quality=80, $interlace = true){
        //创建并设置目录权限
        $savepath = $this->createPath($savepath);
    ...
    //检查并创建保存文件目录
    private function createPath($savePath) {
        //创建相应上传目录
        ...
        if (!is_dir($savePath)) {
            ...
            if (is_dir(base64_decode($savePath))) {
                ...
                if (!mkdir($savePath, 0777, true)) {
                    throw new Exception('目录' . $savePath . '不存在');
                }
            }
        }
