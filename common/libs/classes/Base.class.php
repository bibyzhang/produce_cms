...
class Base{
    ...
    public function get_passage_id($site_from,$article_id=0){
        ...
        $site_from = !empty($site_from) ? $site_from : 'http://www..com';
    ...
    public function makeCache($name, $data, $time=0){
       ...
       $content='<?php if(CACHE_!=\'yes\'){die("Forbidden Access");} ';
       if($time>0){
     ...
     * 获取栏目菜单
     +--------------------------------------------------
     * @param $mid 模块id[1:管理,2:数据管理,3:系统管理]
    ...
    public function RedisGameDataList($state=0){
        $GameInfoDetailTable = $this->OperateTable['GameInfoDetailTable'];
        $game_num = $this->redis->lSize('app..com:game_list');

        if($game_num<0 || $state==1) {

            $this->redis->del('app..com:game_list');//LIST允许重复元素
            ...
            foreach($gameData as $key=>$value){
                ...
                $this->redis->lPush('app..com:game_list', json_encode($info));
    ...
    public function post_curl ($post_url,$post_arr) {
        ...
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);  
        curl_setopt($ch, CURLOPT_REFERER, "www..com");
    ...
    public function get_game_category_passage($game_id,$term_id){
        ...
        foreach($data as $_k=>&$_v)
            $_v['icon'] = 'http://app.static..com' . $_v['icon'];
...
//            );
            ...
            $systemArr .= "'" . 'image_path' . "'" . '=>' . "'" . 'http://app..com/' . "'," . "\n";
            $systemArr .= "'" . 'manage_path' . "'" . '=>' . "'" .'http://app..com//' . "'," . "\n";
            $systemArr .= "'" . 'img_path' . "'" . '=>' . "'" . 'http://app..com' . "'," . "\n";
            $systemArr .= "'" . 'images_path' . "'" . '=>' . "'" . 'http://app.static..com' . "'," . "\n";
            $systemArr .= "'" . 'app_path' . "'" . '=>' . "'" . 'http://app..com/' . "'," . "\n";
