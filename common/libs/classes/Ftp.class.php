...
class ftp {
    ...
    /**
     * 方法：上传文件 
     * @param  $path    -- 本地路径(绝对路径) 
     * @param  $newpath -- 上传路径 (相对路径)
    ...
    /**
     * 判断指定文件是否存在,存在返回true,否则返回false
     * @path -- 路径 
     */
    function file_path_exists($path) {
        $fileList = @ftp_nlist($this->conn_id, dirname($path));
        if (!count($fileList) || !empty($fileList)) {
            return false;
        } else {
