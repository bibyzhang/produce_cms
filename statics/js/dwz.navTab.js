			...
			if (url) {
				if ($tab.hasClass("external")) {
					...
					if ($.isEmptyObject(op.data)) { //获取pagerForm参数
						var $pagerForm = $("#pagerForm", $panel);						
						if(op.pageNum>=1){//修改20131030，赋值当前修改页的页码
							//$panel = #navTab .navTab-panel > div.slice(1,2)
							var a = $panel.selector;
							var b = a.split('.slice(');
							var c = b[1].split(')');
							var d = c[0].split(',');
							
							$(b[0]).slice(d[0],d[1]).find("#pagerForm input[name='pageNum']").attr('value',op.pageNum);
							//console.log(a);
							//console.log(b[0]);
							//console.log(b[1]);
							//console.log(c[0]);
							...
							//console.log(d[0]);
							//console.log(d[1]);
							//console.log($(b[0]).slice(d[0],d[1]).find("#pagerForm input[name='pageNum']").attr('value'));
							//$("#pagerForm input[name='pageNum']").attr('value',op.pageNum);
						}
